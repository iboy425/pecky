...
bool readRawSample(int16_t& ax, int16_t& ay, int16_t& az, int16_t& temp,
  ...
  uint8_t data[14];
  if (!readRegisters(kRegAccelXoutHigh, data, sizeof(data))) {
    return false;
  }
...
bool initializeImu() {
  ...
  sensorWhoAmI = whoAmI;
  if (whoAmI == 0x70) {
    Serial.println("INFO,SENSOR_MODEL,MPU6500_COMPATIBLE");
  } else {
    Serial.println("INFO,SENSOR_MODEL,MPU6050_COMPATIBLE");
  }

  bool ok = true;
  ok &= writeRegister(kRegPowerManagement1, 0x00);  // Wake, internal 8 MHz clock.
  delay(100);
  ok &= writeRegister(kRegConfig, 0x03);             // DLPF about 44 Hz.
  ok &= writeRegister(kRegSampleRateDivider, 0x09);  // 1 kHz / (1 + 9) = 100 Hz.
...
bool calibrateGyroscope() {
  ...
  Serial.println("INFO,CALIBRATION_START,KEEP_SENSOR_STILL");
  ...
  Serial.printf("INFO,CALIBRATION_DONE,%.4f,%.4f,%.4f\n",
                gyroOffsetX, gyroOffsetY, gyroOffsetZ);
  return true;
}

void retrySensorInitialization() {
  scanI2cBus();
  sensorReady = initializeImu();
  if (sensorReady) {
    sensorReady = calibrateGyroscope();
  }

  if (sensorReady) {
    Serial.println(
        "HEADER,t_ms,ax_g,ay_g,az_g,gx_dps,gy_dps,gz_dps,temp_c,pitch_deg,roll_deg,acc_mag_g");
...
void setup() {
  ...
  Serial.println();
  Serial.println("INFO,CHIN_TUCK_HARDWARE_DEMO_V1");
  Serial.printf("INFO,PINS,SDA=%d,SCL=%d,ADDRESS=0x%02X\n",
                kSdaPin, kSclPin, kMpuAddress);

  Wire.begin(kSdaPin, kSclPin, 400000);
  Wire.setTimeOut(50);
  retrySensorInitialization();
}
...
void loop() {
  ...
  Serial.printf("DATA,%lu,%.5f,%.5f,%.5f,%.4f,%.4f,%.4f,%.2f,%.3f,%.3f,%.5f\n",
                millis(), ax, ay, az, gx, gy, gz, temperature,
                pitchDeg, rollDeg, accelerationMagnitude);
}
